...
def create_comment(client_id: int, comment: schemas.CommentCreate, db: Session = Depends(get_db)):
...
def get_comments(client_id: int, skip: int=0, limit: int=20, db: Session = Depends(get_db)):
...
def update_comment(comment_id: int, comment_updated: schemas.CommentUpdate, db: Session = Depends(get_db)):
